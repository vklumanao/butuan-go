...
export function AuthCallbackPage() {
  ...
  useEffect(() => {
    if (!loading && user && profile)
      navigate(getDashboardPath(getActiveRole(profile)), { replace: true });
    ...
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 text-center">
      <div>
        ...
        <h1 className="mt-5 text-2xl font-black">
          We couldn’t complete that confirmation
        </h1>
        <p className="mx-auto mt-3 max-w-md text-slate-600">
          {profileError ||
            "The confirmation link may be invalid or expired. Try logging in, or request a new email from Supabase Authentication."}
        </p>
