...
23. Run `supabase/migrations/021_admin_monthly_analytics.sql`, then run `supabase/verify_admin_monthly_analytics.sql`. This adds an Admin-only Asia/Manila monthly aggregate for registrations, onboarding, request activity and status, disputes, reports, and restriction events without exposing private record details.
24. Run `supabase/migrations/022_admin_analytics_comparisons.sql`, then run `supabase/verify_admin_analytics_comparisons.sql`. This extends the protected monthly response with equivalent prior-period totals, future-month rejection, and a lifecycle-history request funnel.
25. Under **Authentication → URL Configuration**, set the Site URL to `http://localhost:5173` for local development. Add `http://localhost:5173/auth/callback` to the allowed redirect URLs, then add its exact production equivalent after deployment.
26. In Google Cloud, create a Web OAuth client and register `https://YOUR_PROJECT_REF.supabase.co/auth/v1/callback` as an authorized redirect URI. Under **Supabase Authentication → Providers → Google**, enable Google and enter that client ID and secret.
27. Disable public Email authentication in Supabase after confirming the Google flow. The application intentionally exposes no password registration, password login, or password-recovery routes.
